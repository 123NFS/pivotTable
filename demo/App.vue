<template>
  <div id="app" class="container mt-5">
    ...
    <button
      class="mb-5"
      @click="() => (defaultShowSettings = !defaultShowSettings)"
    >
      Toggle Settings
    </button>
    ...
    <button class="btn btn-link" @click="downloadTable('csv')">
      Test to download table in CSV
    </button>
    <button class="btn btn-link" @click="downloadTable('tsv')">
      Test to download table in TSV
    </button>
    <div class="mb-5"></div>
  </div>
</template>
  ...
  data: () => {
    return {
      ...
      fields: {
        ...
        rowFields: [
          {
            getter: (item) => item.districtName,
            label: "District",
            // showHeader:false
          },
          //   {
          //   getter: (item) => item.val.count,
          //   label: "val",
          // },
          // {
          //   getter: (item) => item.val.rashi,
          //   label: "val",
          // },
          // {
